...
which maps it to a parse function.  Two aliases may resolve to the same underlying
type (TiListReal and TiListComplex both alias TiList) yet stay distinct objects,
so they select distinct parsers.  A few aliases (Vectorized, VectorizedReal,
MatrixVectorized) additionally drive element-wise wrapping; PreparsedFunc.__init__
keys off them directly.

@preparse reads the signature (see PreparsedFunc.__init__) and builds a list of
...
parsers are generated by validated(require_X): parse a general expression
...
Runtime imports: core (types, validators, vectorize), environment (Environment),
and parser (ArgParser, whose methods are referenced directly as the parse functions).
"""
...
# ── Registry: vocabulary object → parse function ──────────────────────────────
# Maps each vocabulary object to its parse function.  Env maps to pass_env, which
# returns ArgParser.env without consuming a token; vectorization wrapping is
# decided separately in PreparsedFunc.__init__.
...
class ArgSpec:
	"""One parsed parameter: how to read its value, plus arity metadata."""
	parse: Callable[[ArgParser], Any]
...
class no_arg_command(TiCall):
	...
	def call_with_parser(self, args: Any) -> None:
		if args.has_next:
			raise TiSyntaxError(f"Command takes no arguments, but got: {args.peek()}")
		args.end_cmd()
		self.func(args.env)
...
class PreparsedFunc(TiCall):
	...
	def __init__(self, core_func: Callable, finalizer: Callable[[ArgParser], None]) -> None:
		"""Read core_func's annotated parameters into an ArgSpec schema, and wrap the
		core for vectorization if any slot calls for it.
		...
		"""
		# eval_str=True so this works whether or not the defining module stringizes
		# its annotations (from __future__ import annotations).
		annotations = inspect.get_annotations(core_func, eval_str=True)
		is_vectorized = False
		mat_vec = 0
		schema = []
