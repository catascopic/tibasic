...
class TestSetUpEditor:
	...
	def test_no_args_preserves_existing_data(self):
		env = run('{1,2,3}@ L1')
		run('SetUpEditor', env)
		assert list(env.lists[0].data) == [1, 2, 3]

	def test_with_standard_list(self):
		env = run('SetUpEditor L3')
		assert env.lists[2] is not None   # L3 created
		assert env.lists[0] is None       # L1 untouched
	...
	def test_does_not_overwrite_existing_user_list(self):
		...
		assert list(env.user_lists['SAVE'].data) == [7, 8, 9]

	def test_multiple_args(self):
		env = run('SetUpEditor L1 , L3 , SAVE')
		assert env.lists[0] is not None
		assert env.lists[1] is None       # L2 not requested
		assert env.lists[2] is not None
		assert env.user_lists.get('SAVE') is not None
...
class TestClrList:
	...
	def test_clears_existing_list_to_empty(self):
		...
		ClrList L1
		""")
		assert env.lists[0] is not None   # still defined
		assert len(env.lists[0]) == 0     # but empty
	...
	def test_nonexistent_list_is_silent_noop(self):
		...
		assert env.lists[1] is None       # still None, not created
	...
	def test_multiple_lists(self):
		...
		ClrList L1 , L2
		""")
		assert len(env.lists[0]) == 0
		assert len(env.lists[1]) == 0
...
class TestClrAllLists:
	...
	def test_leaves_undefined_slots_as_none(self):
		...
		ClrAllLists
		""")
		assert env.lists[0] is not None   # was defined, now empty
		assert env.lists[1] is None       # was never defined
...
class TestSortA:
	...
	def test_sorts_ascending(self):
		...
		SortA( L1
		""")
		assert list(env.lists[0].data) == [1, 1, 3, 4, 5]
	...
	def test_dependent_list_reordered_consistently(self):
		...
		""")
		assert list(env.lists[0].data) == [1, 2, 3]
		assert list(env.lists[1].data) == [2, 3, 1]
	...
	def test_two_dependent_lists(self):
		...
		assert list(env.lists[0].data) == [1, 2, 3]
		assert list(env.lists[1].data) == [2, 3, 1]
		assert list(env.lists[2].data) == [30, 10, 20]
...
class TestSortD:
	...
	def test_sorts_descending(self):
		...
		SortD( L1
		""")
		assert list(env.lists[0].data) == [5, 4, 3, 1, 1]
	...
	def test_dependent_list_reordered_consistently(self):
		...
		SortD( L1 , L2
		""")
		assert list(env.lists[0].data) == [3, 2, 1]
		assert list(env.lists[1].data) == [5, 6, 4]
...
class TestFill:
	...
	def test_fill_list(self):
		...
		Fill( 7 , L1
		""")
		assert list(env.lists[0].data) == [7, 7, 7]
	...
	def test_fill_list_with_zero(self):
		...
		Fill( 0 , L1
		""")
		assert list(env.lists[0].data) == [0, 0, 0]
	...
	def test_fill_matrix(self):
		...
		Fill( 9 , [A]
		""")
		assert env.matrices[0].data == [[9, 9], [9, 9]]
