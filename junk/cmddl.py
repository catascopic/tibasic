...
def collect_links_list(soup: BeautifulSoup) -> list[tuple[str, str, str]]:
	"""Parse the <ul> list format used for two-byte token pages.

	Each <li> looks like:
		<strong>7E 00</strong> — <a href="/sequential">Sequential</a>
	The byte code is taken verbatim from <strong>, spaces stripped.
	"""
	seen_hrefs: set[str] = set()
	links: list[tuple[str, str, str]] = []
	...
	for li in soup.find_all("li"):
		...
		anchor = li.find("a")
		if not strong or not anchor:
			continue

		byte_hex = strong.get_text(strip=True).replace(" ", "").upper()
		try:
			int(byte_hex, 16)
		except ValueError:
			continue
...
def main():
	...
	links = collect_links(soup) or collect_links_list(soup)
