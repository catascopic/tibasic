...
token(0x8F, 'ZoomStat')
token(0x90, 'ZoomRcl')
token(0x91, 'PrintScreen', cmd=modes.print_screen)
token(0x92, 'ZoomSto')
token(0x93, 'Text(')
...
token(0xBB26, 'real(',          func=pf.real)
...
token(0xBBF4, '√')
token(0xBBF5, '≛')
SQRT = token(0xBC, '√(',		func=pf.sqrt)
CBRT = token(0xBD, '³√(',		func=pf.cbrt)
token(0xBE, 'ln(',		func=pf.ln)
token(0xBF, '𝑒^(',		func=pf.exp)
token(0xC0, 'log(',		func=pf.log)
...
token(0xCB, 'cosh¹(',	func=pf.acosh)
token(0xCC, 'tanh(',	func=pf.tanh)
token(0xCD, 'tanh¹(',	func=pf.atanh)
token(0xCE, 'If ')
token(0xCF, 'Then')
token(0xD0, 'Else')
token(0xD1, 'While ')
...
token(0xEF05, 'timeCnv(',    func=pf.timecnv)
...
token(0xEF34, 'logBASE(', func=pf.log_base)
