...
pxl_Test    = token(b'\x13', 'pxl-Test(')
augment     = token(b'\x14', 'augment(',     pure_func=pf.augment)
rowSwap     = token(b'\x15', 'rowSwap(',     pure_func=pf.rowswap)
row_plus    = token(b'\x16', 'row+(',        pure_func=pf.row_plus)
times_row   = token(b'\x17', '*row(',        pure_func=pf.times_row)
times_row_plus = token(b'\x18', '*row+(',       pure_func=pf.times_row_plus)
max         = token(b'\x19', 'max(',         pure_func=pf.max)
min         = token(b'\x1a', 'min(',         pure_func=pf.min)
R_to_Pr     = token(b'\x1b', 'R►Pr(',        pure_func=pf.r_pr)
R_to_Ptheta = token(b'\x1c', 'R►Pθ(',        pure_func=pf.r_ptheta)
P_to_Rx     = token(b'\x1d', 'P►Rx(',        pure_func=pf.p_rx)
P_to_Ry     = token(b'\x1e', 'P►Ry(',        pure_func=pf.p_ry)
...
stat_n   = token(b'\x62\x02', 'n')
x_bar    = token(b'\x62\x03', 'ẍ')
Sigma_x  = token(b'\x62\x04', 'Σx')
Sigma_x_sq = token(b'\x62\x05', 'Σx²')
...
y_bar    = token(b'\x62\x0c', 'ȳ')
Sigma_y  = token(b'\x62\x0d', 'Σy')
Sigma_y_sq = token(b'\x62\x0e', 'Σy²')
Sy       = token(b'\x62\x0f', 'Sy')
sigmaY   = token(b'\x62\x10', 'σy')
Sigma_xy = token(b'\x62\x11', 'Σxy')
stat_r   = token(b'\x62\x12', 'r')
Med      = token(b'\x62\x13', 'Med')
Q1       = token(b'\x62\x14', 'Q1')
Q3       = token(b'\x62\x15', 'Q3')
stat_a   = token(b'\x62\x16', 'a')
stat_b   = token(b'\x62\x17', 'b')
stat_c   = token(b'\x62\x18', 'c')
stat_d   = token(b'\x62\x19', 'd')
stat_e   = token(b'\x62\x1a', 'e')
stat_x1  = token(b'\x62\x1b', 'x₁')
stat_x2  = token(b'\x62\x1c', 'x₂')
stat_x3  = token(b'\x62\x1d', 'x₃')
stat_y1  = token(b'\x62\x1e', 'y₁')
stat_y2  = token(b'\x62\x1f', 'y₂')
Stat_y3  = token(b'\x62\x20', 'y₃')
seq_n    = token(b'\x62\x21', '𝑛')
stat_p   = token(b'\x62\x22', 'p')
stat_z   = token(b'\x62\x23', 'z')
stat_t   = token(b'\x62\x24', 't')
chi_sq   = token(b'\x62\x25', 'χ²')
stat_F   = token(b'\x62\x26', '𝐅')
stat_dF  = token(b'\x62\x27', 'df')
p_hat    = token(b'\x62\x28', 'ṕ')
p_hat1   = token(b'\x62\x29', 'ṕ₁')
p_hat2   = token(b'\x62\x2a', 'ṕ₂')
x_bar1   = token(b'\x62\x2b', 'ẍ₁')
Sx1      = token(b'\x62\x2c', 'Sx₁')
n1       = token(b'\x62\x2d', 'n₁')
x_bar2   = token(b'\x62\x2e', 'ẍ₂')
Sx2      = token(b'\x62\x2f', 'Sx₂')
n2       = token(b'\x62\x30', 'n₂')
Sxp      = token(b'\x62\x31', 'Sxp')
lower    = token(b'\x62\x32', 'lower')
upper    = token(b'\x62\x33', 'upper')
stat_s   = token(b'\x62\x34', 's')
r2       = token(b'\x62\x35', 'r²')
R2       = token(b'\x62\x36', 'R²')
Factor_df = token(b'\x62\x37', 'Factor df')
Factor_SS = token(b'\x62\x38', 'Factor SS')
Factor_MS = token(b'\x62\x39', 'Factor MS')
Error_df  = token(b'\x62\x3a', 'Error df')
Error_SS  = token(b'\x62\x3b', 'Error SS')
Error_MS  = token(b'\x62\x3c', 'Error MS')
...
Fin_N     = token(b'\x63\x2b', '𝐍')
I_percent = token(b'\x63\x2c', 'I%')
PV        = token(b'\x63\x2d', 'PV')
PMT       = token(b'\x63\x2e', 'PMT')
FV        = token(b'\x63\x2f', 'FV')
P_per_Y   = token(b'\x63\x30', 'P/Y')
C_per_Y   = token(b'\x63\x31', 'C/Y')
...
Pt_On       = token(b'\x9e', 'Pt-On(')
Pt_Off      = token(b'\x9f', 'Pt-Off(')
Pt_Change   = token(b'\xa0', 'Pt-Change(')
Pxl_On      = token(b'\xa1', 'Pxl-On(')
Pxl_Off     = token(b'\xa2', 'Pxl-Off(')
Pxl_Change  = token(b'\xa3', 'Pxl-Change(')
...
not_        = token(b'\xb8', 'not(',     pure_func=pf.not_)
...
Sigma_prn    = token(b'\xbb\x03', 'Σprn(')
SigmaInt     = token(b'\xbb\x04', 'ΣInt(')
to_Nom       = token(b'\xbb\x05', '►Nom(')
to_Eff       = token(b'\xbb\x06', '►Eff(')
...
chi_sq_Cdf   = token(b'\xbb\x13', 'χ²cdf(',         pure_func=pf.chi2cdf)
